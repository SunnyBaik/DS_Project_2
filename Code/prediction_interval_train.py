...
for i in range(0, X.index[-1]+1, 1000):
    ...
    # 이전 구간이 내려가는 구간일 경우 
        ...
        if (next_s1 >= prev_s1 and (end - start) >= 8000):
            ...
            actual_label = X.iloc[start, 1]
            list_s1_peak_trough.append(trough)
            list_label.append(actual_label) 
            if (trough <= 2100):
                predicted_label = 0
            if (start/100 >= 10):
...
data = {'s1_peak_trough': list_s1_peak_trough, 'label': list_label}
df = pd.DataFrame(data, columns=['s1_peak_trough', 'label'])
df.to_csv(path + 's1_label.csv', index=False)

X['id'] = X.index 
X = X[['id', 's1', 'label', 'predicted_label']].copy() 
...
# trough를 처리하는데 있어서 오류가 큰 듯 
# 오래 값이 유지되면 올라갔다 내려갔다 하는데 이떄 큰 오류 
